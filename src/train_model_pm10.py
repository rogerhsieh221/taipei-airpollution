...
# === Load and preprocess merged data (target: PM10) ===
def load_data():
...
# === Shared Model Training Function (target: PM10) ===
def run_model(df, model_class, model_name, scale=False, model_store=None, **kwargs):
    features = ["AMB_TEMP", "CH4", "CO", "NMHC", "NO", "NO2", "NOx", "O3", "PM2.5",
    ...
    if scale:
        ...
        joblib.dump(scaler, "model/results/scaler_pm10.pkl")
...
# === Markov Chain Model (target: PM10) ===
def run_markov(df):
    try:
        y = df["PM10"].values
...
# === Run All Models (PM10, Linear Regression excluded) ===
def run_all_models():
    print("\n🚀 Starting PM10 prediction using merged_sorted.csv...")
    df = load_data()
    if df.shape[0] < 20:
    ...
    report_path = f"model/results/training_report_pm10_{timestamp}.csv"
    report.to_csv(report_path, index=False)
    print(f"\n📄 Report saved to: {report_path}")

    # Linear Regression already excluded from training; no need to filter
    filtered_report = report

    best_model_name = filtered_report.iloc[0]['model']
    best_model_obj = model_objects.get(best_model_name)
    if best_model_obj:
        best_model_path = f"model/final_model_pm10_{timestamp}.pkl"
